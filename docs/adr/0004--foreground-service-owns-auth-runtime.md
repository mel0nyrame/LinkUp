...
- **状态**：Active / implemented；命令传输方案由 [ADR-0006](0006--deliver-commands-to-running-auth-engine.md) 替代
- **日期**：2026-09-25
...
Dart 入口用 `@pragma('vm:entry-point')` 保留在 AOT 快照中。最初采用 `CallbackHandle` 与 `DartExecutor.executeDartCallback` 下发命令；该方案对已经运行的 Engine 无效，已由 [ADR-0006](0006--deliver-commands-to-running-auth-engine.md) 的 MethodChannel 命令传输替代。启动握手、命令排队和可等待的 UI 调用语义由新方案保留。
...
- **直接通过 MethodChannel 向尚未注册处理器的 Dart isolate 发送命令**：存在启动时序竞争；因此命令在处理器就绪前必须排队。原方案以 callback dispatcher 回避这一问题，但该传输方式已由 [ADR-0006](0006--deliver-commands-to-running-auth-engine.md) 替代。
...
只有在重新评估 Android 前台服务类型政策、后台执行限制、Flutter 多 isolate 成本和通知可见性要求后，才可以更换服务类型或运行时宿主方式。替换实现必须保留单一协调器所有者、状态脱敏边界、sticky 重建语义和 force-stop 不承诺恢复的约束；命令传输的约束由 [ADR-0006](0006--deliver-commands-to-running-auth-engine.md) 负责。
