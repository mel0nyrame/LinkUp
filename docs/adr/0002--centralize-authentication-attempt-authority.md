...
自动模式的 ACID 选择顺序固定为 Reality 捕获、已保存值、认证服务器根目录探测；手动模式只使用已保存值。候选携带来源和网络世代。登录失败、异常、取消、已经在线、网络世代变化或 `rad_user_info` 未确认在线时，候选失效且不写入配置。只有 Portal 成功后再次确认在线，协调器才通过局部 `ConfigUpdate(acid: ...)` 保存本轮实际使用的 ACID。
...
- 协调器状态流和单飞 Future 现在是认证运行时的事实来源；后续监控退避和 Android 后台服务可以复用该边界，而不必复制协议流程。
- 当前 `ConfigRepository` 会把持久化空 ACID 归一化为默认值 `1`；根目录探测只在配置源明确返回空值时触发。
